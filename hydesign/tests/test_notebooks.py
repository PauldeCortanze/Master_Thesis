...
def test_notebooks(notebook):
    ...
    if (str(Path(notebook.filename).relative_to(os.path.dirname(hydesign.__file__) + "/../docs/notebooks/")) in
            ['Optimization.ipynb']):
        return
...
if __name__ == '__main__':
    ...
    for n in notebooks:
        ...
        if not os.path.exists(test_report_path):
            ...
            print(f"Skipping {n.filename} as test report already exists at {test_report_path}")

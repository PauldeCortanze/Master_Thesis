...
def run_evaluation_design_1_P2X():
    output_df = pd.read_csv(
        tfp+'France_good_wind_design_P2X.csv',
        index_col=0, 
        parse_dates = True)
    examples_sites = pd.read_csv(f'{examples_filepath}examples_sites.csv', index_col=0)
    name = 'France_good_wind'
    ...
    x = [clearance, sp, p_rated, Nwt, wind_MW_per_km2, \
    solar_MW, surface_tilt, surface_azimuth, solar_DCAC, \
    b_P, b_E_h , cost_of_batt_degr, ptg_MW, HSS_kg]

    outs = hpp.evaluate(*x)
    hpp.evaluation_in_csv(os.path.join(tfp + 'tmp', 'test_eval_design_1_P2X'), longitude, latitude, altitude, x, outs)
    return outs
...
def update_test_design_1_P2X():
    output_df = pd.read_csv(
        ...
        index_col=0, 
        parse_dates = True)
    run_evaluation_design_1_P2X()
    eval_df = pd.read_csv(os.path.join(tfp + 'tmp', 'test_eval_design_1_P2X.csv'))
    output_df['Design 1'] = eval_df.T[0]
    output_df.to_csv(tfp+'France_good_wind_design_P2X.csv')
...
def load_evaluation_design_1_P2X():
    output_df = pd.read_csv(
        ...
        index_col=0, 
        parse_dates = True)
    load_file = np.array(output_df.iloc[17:]['Design 1'])
...
def run_evaluation_design_2_P2X():
    ...
    b_E_h  = output_df.loc['b_E_h [h]','Design 2']
    cost_of_batt_degr = output_df.loc['cost_of_battery_P_fluct_in_peak_price_ratio','Design 2']
    ptg_MW = output_df.loc['PTG [MW]','Design 2']
    HSS_kg = output_df.loc['HSS [kg]','Design 2']
    ...
    outs = hpp.evaluate(*x)
    hpp.evaluation_in_csv(os.path.join(tfp + 'tmp', 'test_eval_design_2_P2X'), longitude, latitude, altitude, x, outs)
...
def update_test_design_2_P2X():
    output_df = pd.read_csv(
        ...
        index_col=0, 
        parse_dates = True)
    run_evaluation_design_2_P2X()
    eval_df = pd.read_csv(os.path.join(tfp + 'tmp', 'test_eval_design_2_P2X.csv'))
    output_df['Design 2'] = eval_df.T[0]
    output_df.to_csv(tfp+'France_good_wind_design_P2X.csv')
...
def load_evaluation_design_2_P2X():
    output_df = pd.read_csv(
        ...
        index_col=0, 
        parse_dates = True)
    load_file = np.array(output_df.iloc[17:]['Design 2'])
...
def run_evaluation_design_3_P2X():
    ...
    b_E_h  = output_df.loc['b_E_h [h]','Design 3']
    cost_of_batt_degr = output_df.loc['cost_of_battery_P_fluct_in_peak_price_ratio','Design 3']
    ptg_MW = output_df.loc['PTG [MW]','Design 3']
    HSS_kg = output_df.loc['HSS [kg]','Design 3']
    ...
    outs = hpp.evaluate(*x)
    hpp.evaluation_in_csv(os.path.join(tfp + 'tmp', 'test_eval_design_3_P2X'), longitude, latitude, altitude, x, outs)
...
def update_test_design_3_P2X():
    output_df = pd.read_csv(
        ...
        index_col=0, 
        parse_dates = True)
    run_evaluation_design_3_P2X()
    eval_df = pd.read_csv(os.path.join(tfp + 'tmp', 'test_eval_design_3_P2X.csv'))
    output_df['Design 3'] = eval_df.T[0]
    output_df.to_csv(tfp+'France_good_wind_design_P2X.csv')
...
def load_evaluation_design_3_P2X():
    output_df = pd.read_csv(
        ...
        index_col=0, 
        parse_dates = True)
    load_file = np.array(output_df.iloc[17:]['Design 3'])

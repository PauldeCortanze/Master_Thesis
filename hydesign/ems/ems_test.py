...
class ems:
    ...
    def __init__(
        ...
        ems_type="cplex",
        mdf_t=None,
        disp_value=100e-3,  # euro per kg of CO2 displaced, for calculating displ_emissions_cost in the EMS variable OPEX. This is a placeholder value and should be updated with a more accurate estimate.
    ):
    ...
    def compute(
        ...
        **kwargs,
        ...
        P_HPP_ts, P_curtailment_ts, P_charge_discharge_ts, E_SOC_ts, penalty_ts, displ_ts = (
            ems_WSB(
        ...
        outputs["displ_t_ext"] = expand_to_lifetime(
            displ_ts, life_y=self.life_y, intervals_per_hour=self.intervals_per_hour
        )
        return (
            outputs["wind_t_ext"],
            outputs["solar_t_ext"],
...
def ems_cplex(
    ...
    for ib, batch in enumerate(batches):
        ...
        displ_ts_batch = mdf_t_sel * P_HPP_ts_batch
        ...
        displ_ts[batch] = displ_ts_batch

    E_SOC_ts[-1] = E_SOC_ts[0]

    return P_HPP_ts, P_curtailment_ts, P_charge_discharge_ts, E_SOC_ts, penalty_ts, displ_ts
...
def ems_cplex_parts(
    ...
    mdl.maximize(
        # revenues and OPEX
        mdl.sum(price_ts[t] * P_HPP_t[t] 
                + disp_value * mdf_t[t] * P_HPP_t[t]
                for t in time)

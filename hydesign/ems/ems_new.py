...
class ems:
    ...
    def __init__(
        ...
        # emission_weight is stored here only to expose it as a readable attribute;
        # the actual value is passed at compute() time via the OpenMDAO input.
    ...
    def compute(
        ...
        **kwargs,
        ...
        """
        Parameters (additions vs original)
        ------------------------------------
        mdf_t : array-like of shape (N_time,) or None
            Marginal displacement factor [kgCO2eq/MWh] aligned to the
            simulation time index.  None (or all-zeros) disables the carbon
            signal.
        emission_weight : float  [€/tCO2eq]
            Carbon shadow-price injected into the LP objective as
            ``emission_weight/1000 * mdf[t]`` [€/MWh].  Zero reproduces the
            original economic-only behaviour.
        """
        ...
        battery_depth_of_discharge = np.asarray(
            battery_depth_of_discharge).item()
        battery_charge_efficiency = np.asarray(
            battery_charge_efficiency).item()
        ...
        emission_weight = float(np.asarray(emission_weight).item())

        # Build a synthetic time index to avoid problems with time-series
        # indexing in ems
        ...
        # Attach MDF to the DataFrame so it shares the same synthetic index.
        # If mdf_t is None or all-zeros, the carbon signal is inactive.
        if mdf_t is not None and emission_weight != 0.0:
            WSPr_df["mdf_t"] = np.asarray(mdf_t)
        else:
            WSPr_df["mdf_t"] = 0.0
    ...
    def __init__(self, **insta_inp):
        ...
        super().__init__(
            inputs=[
                ...
                # ----------------------------------------------------------
                # Carbon-offset dispatch signal (optional)
                # ----------------------------------------------------------
                (
                    "mdf_t",
                    {
                        "desc": "Marginal displacement factor time series [kgCO2eq/MWh]. "
                                "Zero disables the carbon signal.",
                        "shape": [model.N_time],
                        "val": 0.0,
                    },
                ),
                (
                    "emission_weight",
                    {
                        "desc": "Carbon shadow-price [€/tCO2eq]. "
                                "Scales the MDF signal in the LP objective as "
                                "emission_weight/1000 * mdf[t] [€/MWh]. "
                                "Zero reproduces the original economic-only dispatch.",
                        "val": 0.0,
                    },
                ),
            ],
...
def ems_cplex(
    ...
    # [0, 0.8]. For higher values might cause errors
    ...
    # Ensure mdf_ts is a Series aligned to wind_ts if provided
    if mdf_ts is None:
        mdf_ts = pd.Series(0.0, index=wind_ts.index)
    elif not isinstance(mdf_ts, pd.Series):
        mdf_ts = pd.Series(np.asarray(mdf_ts), index=wind_ts.index)
...
def ems_cplex_parts(
    ...
    cost_of_battery_P_fluct_in_peak_price_ratio : cost of battery power fluctuations computed as a peak price ratio
    n_full_power_hours_expected_per_day_at_peak_price : Penalty occurs if number of full power hours expected per day at peak price are not reached
    mdf_ts : pd.Series or None
        Marginal displacement factor [kgCO2eq/MWh] aligned to price_ts.index.
        When None or all-zeros the term has no effect on dispatch.
    emission_weight : float  [€/tCO2eq]
        Carbon shadow-price.  Added to the effective price seen by the LP as
        ``emission_weight / 1000 * mdf[t]``  [€/MWh].
        Zero → identical to the original economic-only objective.
    ...
    # ------------------------------------------------------------------
    # Build the effective price seen by the LP:
    #   effective_price[t] = market_price[t]
    #                        + emission_weight [€/tCO2eq]
    #                          * mdf[t] [kgCO2eq/MWh] / 1000 [kg/t]
    # This adds a carbon bonus on top of the electricity price at each
    # timestep, proportional to how much grid carbon the HPP displaces.
    # When emission_weight == 0 the term vanishes and behaviour is
    # identical to the original single-objective EMS.
    # ------------------------------------------------------------------

    if mdf_ts is not None and emission_weight != 0.0:
        carbon_bonus = emission_weight / 1000.0 * np.asarray(mdf_ts)
    else:
        carbon_bonus = 0.0

    effective_price_ts = pd.Series(
        price_ts.values + carbon_bonus,
        index=price_ts.index,
    )
    ...
    mdl.maximize(
        ...
        mdl.sum(effective_price_ts[t] * P_HPP_t[t] for t in time)
        - penalty  # Add cost for rapid charge-discharge for limiting the battery life use
        - mdl.sum(
            fabs(
                P_charge_discharge[t + pd.Timedelta("1hour")] - P_charge_discharge[t])
            * cost_of_battery_P_fluct_in_peak_price_ratio
            * price_ts_to_max[t]
            for t in time[:-1]
        )
    ...
    for t in time:
        ...
        mdl.add_constraint(E_SOC_t[tt] == E_SOC_t[t] -
        ...
        mdl.add_constraint(
            P_charge_discharge[t] <= P_batt_MW * charge_efficiency)
        mdl.add_constraint(
            P_charge_discharge[t] >= -P_batt_MW / charge_efficiency)
...
def ems_Wind_Solar_Battery_Pyomo_parts(
    ...
    model.P_HPP_t = pyo.Var(
        model.IDX1, domain=pyo.NonNegativeReals, bounds=(
            0, hpp_grid_connection)
    )
    ...
    model.e_penalty = pyo.Var(
        model.IDX2, domain=pyo.Reals, bounds=(-1e12, 1e12))
    ...
    model.P_battery_delta = pyo.Var(
        model.IDX5, domain=pyo.Reals, bounds=(-1e12, 1e12))
    ...
    for count in range(len(time)):
        ...
        P_charge_discharge_ts.append(
            model.P_charge_discharge_with_eff_t[count]())
...
def operation_solar_batt_deg(
    ...
    for i in range(len(b_t_less_sol)):
        if b_t_less_sol[i] < 0:  # charging
            b_E_SOC_t_sat[i + 1] = (
                b_E_SOC_t_sat[i] - b_t_less_sol[i] *
                dt * battery_charge_efficiency
            )
    ...
    for i in range(len(b_t)):
        if b_t[i] < 0:
            if change_BES_charging == "proportional":
                if wind_solar_t[i] != 0:
                    # Try to keep the ratio of b_t[i] / wind_solar_t[i]  SoC to the maximum
                    b_t_less_sol[i] = (b_t[i] / wind_solar_t[i]
                                       ) * (wind_solar_t_deg[i])
...
def operation_constant_output(
    ...
    for ii in range(iterations):
        ...
        P_curt_deg = P_curt_deg + \
            df_aux["P_to_b_removed_to_curtailment"].values

...
class ems_pp:
    ...
    def compute(self, wind_t, solar_t, price_t, b_P, b_E, G_MW, battery_depth_of_discharge, battery_charge_efficiency, peak_hr_quantile, cost_of_battery_P_fluct_in_peak_price_ratio, n_full_power_hours_expected_per_day_at_peak_price, **kwargs):
        ...
        outputs['penalty_t'] = expand_to_lifetime(
            ...
            intervals_per_hour = self.intervals_per_hour, weeks_per_season_per_year = self.weeks_per_season_per_year)
        return outputs['wind_t_ext'], outputs['solar_t_ext'], outputs['price_t_ext'], \
               outputs['hpp_t'], outputs['hpp_curt_t'], outputs['b_t'], \
               outputs['b_E_SOC_t'], outputs['penalty_t']
...
class ems(ComponentWrapper):
    def __init__(self, **insta_inp):
        ...
        super().__init__(
            inputs=[
                ('wind_t', {'units': 'MW', 'desc': 'WPP power time series', 'shape':[ems_pp_model.N_time]}),
                ('solar_t', {'units': 'MW', 'desc': 'PVP power time series', 'shape':[ems_pp_model.N_time]}),
                ...
                ('battery_depth_of_discharge', {'units': 'MW', 'desc': 'battery depth of discharge'}),
                ('battery_charge_efficiency', {'desc': 'battery charge efficiency'}),
                ('peak_hr_quantile', {'desc': "Quantile of price tim sereis to define peak price hours (above this quantile).\n"+
                ...
                ('n_full_power_hours_expected_per_day_at_peak_price', {'desc': "Pnealty occurs if nunmber of full power hours expected per day at peak price are not reached."}),
            ],
            outputs=[
                ('wind_t_ext', {'units': 'MW', 'desc': 'WPP power time series', 'shape':[ems_pp_model.life_h]}),
                ('solar_t_ext', {'units': 'MW', 'desc': 'PVP power time series', 'shape':[ems_pp_model.life_h]}),
                ('price_t_ext', {'desc': 'Electricity price time series', 'shape':[ems_pp_model.life_h]}),
                ('hpp_t', {'units': 'MW', 'desc': 'HPP power time series', 'shape':[ems_pp_model.life_h]}),
                ('hpp_curt_t', {'units': 'MW', 'desc': 'HPP curtailed power time series', 'shape':[ems_pp_model.life_h]}),
                ('b_t', {'units': 'MW', 'desc': 'Battery charge/discharge power time series', 'shape':[ems_pp_model.life_h]}),
                ('b_E_SOC_t', {'desc': 'Battery energy SOC time series', 'shape':[ems_pp_model.life_h + 1]}),
                ('penalty_t', {'desc': "penalty for not reaching expected energy productin at peak hours", 'shape':[ems_pp_model.life_h]}),
            ],
...
class ems_long_term_operation_pp:
    ...
    def compute(self, SoH, wind_t_ext_deg, solar_t_ext_deg, wind_t_ext, solar_t_ext, price_t_ext, b_P, b_E, G_MW, battery_depth_of_discharge, battery_charge_efficiency, hpp_curt_t, b_t, b_E_SOC_t,peak_hr_quantile, n_full_power_hours_expected_per_day_at_peak_price, load_min=None, **kwargs):
        ...
        return outputs['hpp_t_with_deg'], outputs['hpp_curt_t_with_deg'], \
               outputs['b_t_with_deg'], outputs['b_E_SOC_t_with_deg'], \
                outputs['penalty_t_with_deg'], \
                outputs['total_curtailment_with_deg'], outputs['total_curtailment']

class ems_long_term_operation(ComponentWrapper):
    def __init__(self, **insta_inp):
        ems_long_term_operation_pp_model = ems_long_term_operation_pp(**insta_inp)
        super().__init__(
            inputs=[
                ('SoH', {'desc': 'Battery state of health at discretization levels', 'shape': [ems_long_term_operation_pp_model.life_intervals]}),
                ('wind_t_ext_deg', {'units': 'MW', 'desc': 'Wind time series including degradation', 'shape': [ems_long_term_operation_pp_model.life_intervals]}),
                ('solar_t_ext_deg', {'units': 'MW', 'desc': 'PV time series including degradation', 'shape': [ems_long_term_operation_pp_model.life_intervals]}),
                ('wind_t_ext', {'units': 'MW', 'desc': 'WPP power time series', 'shape': [ems_long_term_operation_pp_model.life_intervals]}),
                ('solar_t_ext', {'units': 'MW', 'desc': 'PVP power time series', 'shape': [ems_long_term_operation_pp_model.life_intervals]}),
                ('price_t_ext', {'desc': 'Electricity price time series', 'shape': [ems_long_term_operation_pp_model.life_intervals]}),
                ('b_P', {'units': 'MW', 'desc': 'Battery power capacity'}),
                ('b_E', {'desc': 'Battery energy storage capacity'}),
                ('G_MW', {'units': 'MW', 'desc': 'Grid capacity'}),
                ('battery_depth_of_discharge', {'units': 'MW', 'desc': 'battery depth of discharge'}),
                ('battery_charge_efficiency', {'desc': 'battery charge efficiency'}),
                ('hpp_curt_t', {'units': 'MW', 'desc': "HPP curtailed power time series", 'shape': [ems_long_term_operation_pp_model.life_intervals]}),
                ('b_t', {'units': 'MW', 'desc': "Battery charge/discharge power time series", 'shape': [ems_long_term_operation_pp_model.life_intervals]}),
                ('b_E_SOC_t', {'desc': "Battery energy SOC time series", 'shape': [ems_long_term_operation_pp_model.life_intervals + 1]}),
                ('peak_hr_quantile', {'desc': "Quantile of price time series to define peak price hours (above this quantile).\n"+
                                              "Only used for peak production penalty and for cost of battery degradation."}),
                ('n_full_power_hours_expected_per_day_at_peak_price', {'desc': "Pnealty occurs if number of full power hours expected per day at peak price are not reached."}),
                ('load_min', {'desc': 'Minimum electrical load to meet'}),
            ],
            outputs=[
                ('hpp_t_with_deg', {'units': 'MW', 'desc': 'HPP power time series', 'shape': [ems_long_term_operation_pp_model.life_intervals]}),
                ('hpp_curt_t_with_deg', {'units': 'MW', 'desc': 'HPP curtailed power time series', 'shape': [ems_long_term_operation_pp_model.life_intervals]}),
                ('b_t_with_deg', {'units': 'MW', 'desc': 'Battery charge/discharge power time series', 'shape': [ems_long_term_operation_pp_model.life_intervals]}),
                ('b_E_SOC_t_with_deg', {'desc': 'Battery energy SOC time series', 'shape': [ems_long_term_operation_pp_model.life_intervals + 1]}),
                ('penalty_t_with_deg', {'desc': "penalty for not reaching expected energy production at peak hours", 'shape': [ems_long_term_operation_pp_model.life_intervals]}),
                ('total_curtailment_with_deg', {'units': 'GW*h', 'desc': "total curtailment in the lifetime after degradation"}),
                ('total_curtailment', {'units': 'GW*h', 'desc': "total curtailment in the lifetime"}),
            ],
            function=ems_long_term_operation_pp_model.compute,
            partial_options=[{'dependent': False, 'val': 0}],
        )

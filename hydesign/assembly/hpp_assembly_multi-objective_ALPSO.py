...
#  (environmental).
#
#  Changes vs. original single-objective version
#  -----------------------------------------------
#  1. load_mdf_aligned() helper — loads & aligns MDF CSV (day-first dates,
#     2023-2024) to the simulation time index, handles week-sampling.
#  2. CarbonOffsetComp — plain om.ExplicitComponent that receives the
#     lifetime hpp_t (shape life_h) directly from the EMS and the tiled MDF
#     array of the same shape, and outputs annual_carbon_offset [tCO2eq/yr].
#  3. hpp_base.get_defaults() now includes mdf_fn=None.
#  4. hpp_base.__init__() loads & stores mdf_t (or None).
#  5. hpp_model.__init__() appends CarbonOffsetComp to the problem *after*
#     get_prob(), connects hpp_t → carbon_offset.hpp_t_co explicitly, and
#     tiles mdf_t to lifetime length before set_val().
#  6. setup_optimization() adds annual_carbon_offset as second objective
#     when use_carbon_offset is True.
#  7. evaluate(), run_optimization(), save_nsga2_results(), and
#     run_optimization_seed() updated to handle the optional second objective.
#  8. list_out_vars extended with "Annual carbon offset [tCO2eq/yr]" when MDF
#     is active.
# =============================================================================
...
class CarbonOffsetComp(om.ExplicitComponent):
    ...
    mdf_t : np.ndarray, shape (life_h,)
        Marginal displacement factor [kgCO2eq/MWh] tiled over the lifetime.

    Outputs
    -------
    annual_carbon_offset : float
        Mean annual carbon offset [tCO2eq/yr] = sum(hpp_t_co * mdf_t) / life_y / 1e3
    """
    ...
    def setup(self):
        ...
        self.add_input('mdf_t',    shape=(life_h,),              val=0.0)
    ...
    def compute(self, inputs, outputs):
        ...
        mdf_t  = inputs['mdf_t']             # kgCO2eq / MWh
        life_y = self.options['life_y']

        # kgCO2eq / hour, summed over lifetime, divided by years → kg/yr → t/yr
...
class hpp_base:
    def __init__(self, sim_pars_fn, variables, defaults={}, **kwargs):
        ...
        # ------------------------------------------------------------------
        # MDF loading (carbon offset signal)
        # ------------------------------------------------------------------
        mdf_t  = None
...
class hpp_model(hpp_base):
    ...
    def __init__(self, sim_pars_fn, **kwargs):
        ...
        # Whether the carbon offset objective is active
        ...
        # and connect explicitly to the lifetime hpp_t (shape life_h).
        # ------------------------------------------------------------------
        if self.use_carbon_offset:
            prob.model.add_subsystem(
                "carbon_offset",
                CarbonOffsetComp(life_h=life_h, life_y=life_y),
                promotes_inputs=["mdf_t"],              # mdf_t is unique — safe to promote
                promotes_outputs=["annual_carbon_offset"],
            )
            # Explicit connection: promoted 'hpp_t' (lifetime, shape life_h)
            # → carbon_offset.hpp_t_co.  Uses the promoted name directly so
            # OpenMDAO resolves it to the correct ems_comp output.
        ...
        self.setup_optimization(
            ...
            Swarmsize=kwargs.get('Swarmsize'),
        ...
        # Pass tiled MDF values (lifetime length) into CarbonOffsetComp
        if self.use_carbon_offset:
            mdf_tiled = np.tile(mdf_t, int(life_y))   # (N_time,) → (life_h,)
            prob.set_val("carbon_offset.mdf_t", mdf_tiled)
    ...
    # ------------------------------------------------------------------
    def setup_optimization(self, seed=0, Swarmsize=15, MaxIter=10, c1=2, c2=1, w1=0.99, w2=0.55):
        ...
        # Objectives
        # -------------------------------------------------------------------
        # Objective 1 — Economic: maximise NPV/CAPEX
        model.add_objective('NPV_over_CAPEX', scaler=-1)

        # Objective 2 — Environmental: maximise annual carbon offset
        # (only when MDF data is available)
        # if self.use_carbon_offset:
        model.add_objective('annual_carbon_offset', scaler=-1)

        # -------------------------------------------------------------------
        # ALPSO driver (handles multiple objectives natively — Pareto front)
        # -------------------------------------------------------------------
        prob.driver = om.pyOptSparseDriver()
        prob.driver.options['optimizer']          = 'ALPSO'
        prob.driver.opt_settings['SwarmSize']     = Swarmsize
        prob.driver.opt_settings['maxOuterIter']  = MaxIter
        prob.driver.opt_settings['maxInnerIter']  = 1
        prob.driver.opt_settings['minInnerIter']  = 1
        prob.driver.opt_settings['stopCriteria']  = 0


        output_dir = 'optimization_results_multi-objective'
        os.makedirs(output_dir, exist_ok=True)
        self.filepath = os.path.join(
            output_dir, f"results_ALPSO_Carbon_seed{seed}_c1{c1}_c2{c2}.sql")
        self.filepath_csv = os.path.join(
            output_dir, f"history_ALPSO_Carbon_seed{seed}_c1{c1}_c2{c2}.csv")
...
def run_optimization_seed(args):
    try:
        (seed, Swarmsize, MaxIter, c1, c2, w1, w2,
         latitude, longitude, altitude,
         sim_pars_fn, input_ts_fn, variables, mdf_fn) = args

        print(f"[Run {seed}] seed={seed} PID={os.getpid()}", flush=True)

        base_dir   = os.path.abspath('optimization_results_multi')
        worker_dir = os.path.join(
            base_dir,
            f'Multi_worker_run_c1{c1}_c2{c2}_seed{seed}_pid{os.getpid()}')
        ...
        hpp = hpp_model(
            ...
            mdf_fn=mdf_fn,       # None → single-objective run
        )
        ...
        print(f"[Run {seed}] Done → NPV/CAPEX={result['NPV_over_CAPEX']:.4f}",
              flush=True)
        if hpp.use_carbon_offset:
            print(f"[Run {seed}]        annual_carbon_offset="
                  f"{result['annual_carbon_offset']:.0f} tCO2eq/yr", flush=True)
        return result
...
if __name__ == "__main__":
    ...
    run_optimization_seed((0, 15, 10, 2, 1, 0.99, 0.55, latitude, longitude, altitude,
                           sim_pars_fn, input_ts_fn, variables, mdf_fn))
    ...
    print(f"Launching {len(args_list)} runs on {n_processes} processes...",
          flush=True)
    ...
    for r in results:
        line = (f"Run {r['run']} (Pc={r['Pc']}, Pm={r['Pm']}) "
                f"→ NPV/CAPEX={r['NPV_over_CAPEX']:.4f}")
        if 'annual_carbon_offset' in r:
            line += f"  |  annual_CO2_offset={r['annual_carbon_offset']:.0f} tCO2eq/yr"
        print(line)
    '''
...
class hpp_model(hpp_base):
    ...
    def setup_optimization(
        ...
        # ── NSGA2_Driver ───────────────────────────────────────────────────
        output_dir = 'optimization_results_multi-objective_NSGA'
        os.makedirs(output_dir, exist_ok=True)

        history_csv = os.path.join(
            output_dir,
            f"swarm_history_NSGA_seed{seed}_ew{emission_weight}.csv"
        )
        ...
        self.filepath = os.path.join(
            output_dir,
            f"results_NSGA_seed{seed}_ew{emission_weight}.sql"
        )
        self.filepath_csv = os.path.join(
            output_dir,
            f"history_NSGA_seed{seed}_ew{emission_weight}.csv"
        )
...
def run_optimization_seed(args):
    ...
    try:
        ...
        pareto_csv = os.path.join(
            ...
            f"pareto_front_NSGA_seed{seed}_ew{emission_weight}.csv"
        )

...
class hpp_base:
    ...
    def get_prob(comps):
        ...
        #     if meta['var_type'] == 'fixed':
        #         ivc.add_output(var, val=meta['value'])
        #     elif meta['var_type'] == 'design':
        #         lower, upper = meta['limits']
        #         ivc.add_output(var, val=(lower + upper) / 2)

        # # Add IndepVarComp to the model
        ...
        for c in comps:
            if len(c) == 2:
                ...
                prob.model.add_subsystem(
                    c[0], c[1], promotes_inputs=input_list, promotes_outputs=[
                        k for k in c[1].output_keys]
                )
...
class hpp_model(hpp_base):
    ...
    def __init__(self, sim_pars_fn, **kwargs):
        ...
        comps = [
            ("mapping",
                HPPMappingComp(),
                {
                    ...
                    "b_P_out": "b_P",
                    "cost_of_battery_P_fluct_in_peak_price_ratio_out": "cost_of_battery_P_fluct_in_peak_price_ratio",
                },
             ),
            (
                "abl",
        ...
        inputs_df["type"] = "input"
        outputs_df["type"] = "output"

        # Combine into one DataFrame
        combined_df = pd.concat([inputs_df, outputs_df], ignore_index=True)
    ...
    def run_optimization(self):
        ...
        results['objective'] = - self.prob.get_val('NPV_over_CAPEX')

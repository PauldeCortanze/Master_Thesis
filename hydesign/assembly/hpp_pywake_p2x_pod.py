...
from hydesign.ems.ems_P2X import ems_P2X as ems_P2X_pp, ems_long_term_operation_p2x as ems_long_term_operation_p2x_pp
...
from hydesign.costs.costs import battery_cost as battery_cost_pp, pvp_cost as pvp_cost_pp, shared_cost as shared_cost_pp, wpp_cost as wpp_cost_pp, ptg_cost as ptg_cost_pp
        ...
        **kwargs
        ...
        def pywake_ref(ws, wd, **kwargs):
            ...
            flow_map = compute_flow_map(
                sim_res, x_grid, y_grid, z_grid, axial_wind=False, time=sim_res["time"]#[:50]
            )
            
            power_demand = np.full((flow_map["wt"].size, flow_map["time"].size), 100.0)
            yaw = np.broadcast_to((sim_res["yaw"].values)[:, np.newaxis], power_demand.shape)
            ...
            loads_ref_ext = expand_to_lifetime(loads.values, life_y=life_y, intervals_per_hour=intervals_per_hour, axis=1)
        ...
        def pywake(ws, wd, yaw, loads_ref_ext, **kwargs):
            ...
            loads_rel_ext = expand_to_lifetime(loads, life_y=life_y, intervals_per_hour=intervals_per_hour, axis=1) / loads_ref_ext

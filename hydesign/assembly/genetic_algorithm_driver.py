...
class SimpleGADriver(Driver):
    ...
    def _declare_options(self):
        ...
        self.options.declare('early_stop_tol', default=1e-5, lower=0.,
                             desc='Convergence tolerance for early stopping.')
        self.options.declare('early_stop_gen', default=3, lower=1,
                             desc='Number of consecutive generations with improvement below tolerance for early stopping.')
    ...
    def _setup_driver(self, problem):
        ...
        self._ga = GeneticAlgorithm(self.objective_callback, comm=comm, model_mpi=model_mpi,
                                    early_stop_tol=self.options['early_stop_tol'],
                                    early_stop_gen=self.options['early_stop_gen'])
...
#        return comm
    ...
    def run(self):
        ...
        ga.early_stop_tol = self.options['early_stop_tol']
        ga.early_stop_gen = self.options['early_stop_gen']
        pop_size = self.options['pop_size']
        max_gen = self.options['max_gen']
        user_bits = self.options['bits']
        # if None, it will be calculated in execute_ga()
        Pm = self.options['Pm']
        Pc = self.options['Pc']
    ...
    def objective_callback(self, x, icase):
        ...
        with RecordingDebugging('SimpleGA', self.iter_count, self) as rec:
            ...
            if is_single_objective:  # Single objective optimization
                # First and only key in the dict
                obj = next(itervalues(obj_values))
            ...
            if penalty == 0:
                ...
                fun = obj + penalty * \
                    sum(np.power(constraint_violations, exponent))
        ...
        outputs = {}
        for name in model._var_abs2meta['output']:
            outputs[name] = model._outputs[name]
        # print("Functions calculated")
        # print(x)
        # print(obj)
        return fun, success, icase, outputs
...
class GeneticAlgorithm(object):
    ...
    def __init__(self, objfun, comm=None, model_mpi=None, early_stop_tol=1e-5, early_stop_gen=3):
        """
    ...
    def execute_ga(self, x0, vlb, vub, vob, bits, pop_size, max_gen, random_state, Pm=None, Pc=0.5):
        ...
        # early_stop_tol = self.objfun._driver.options['early_stop_tol']
        # early_stop_gen = self.objfun._driver.options['early_stop_gen']
...
#            else:
            ...
            for ii in range(self.npop):
                ...
                if np.any(x - vob > 0):
                    ...
                    fitness[ii], success, _, outputs = self.objfun(x, 0)
            ...
            best_fitness_history.append(min_fit)
            ...
            if generation > 0:
                improvement = mean_fitness_history[-2] - \
                    mean_fitness_history[-1]
                if improvement < self.early_stop_tol:
                    no_improvement_count += 1
                else:
                    no_improvement_count = 0

                if no_improvement_count >= self.early_stop_gen:
                    print(
                        f"Early stopping at generation {generation} due to convergence.")
                    break
    ...
    def tournament(self, old_gen, fitness):
        ...
        for j in range(2):
            ...
            i_min = np.argmin(
                np.array([[fitness[idx]], [fitness[idx + 1]]]), axis=0)
            selected = i_min + idx

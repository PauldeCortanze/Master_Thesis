...
#     def compute(self, price_t_ext, hpp_t_with_deg, penalty_t,
#                 CAPEX_w, OPEX_w, CAPEX_s, OPEX_s, CAPEX_b, OPEX_b, CAPEX_el, OPEX_el,
#                 wind_WACC, solar_WACC, battery_WACC, tax_rate, **kwargs):
...
#             df_out['revenue'] = np.insert(np.sum(np.split(df['hpp_t'].values*price_t_ext, 25), axis=1), 0, 0)
#             df_out['price_eq'] = df_out['revenue'] / df_out['production']
#             df_out['actual_mean_price'] = np.insert(np.mean(np.split(price_t_ext, 25),axis=1), 0, 0)
...
#         return outputs['CAPEX'], outputs['OPEX'], outputs['revenues'], \
#                outputs['NPV'], outputs['IRR'], outputs['NPV_over_CAPEX'], outputs['LCOE'], \
#                outputs['mean_AEP'], outputs['penalty_lifetime'], outputs['break_even_PPA_price']
...
class finance_pp:
    ...
    def compute(self, hpp_t_with_deg, penalty_t, price_t_ext, CAPEX_w, CAPEX_s, CAPEX_b, CAPEX_el, 
                OPEX_w, OPEX_s, OPEX_b, OPEX_el, wind_WACC, solar_WACC, battery_WACC, tax_rate, **kwargs):
        """ Calculating the financial metrics of the hybrid power plant project.
...
class finance(ComponentWrapper):
    def __init__(self, **insta_inp):
        ...
        super().__init__(inputs=[('price_t_ext', {'shape':[FIN.life_h]}),
                                ('hpp_t_with_deg', {'shape':[FIN.life_h], 'units':'MW'}),
                                ('penalty_t', {'shape':[FIN.life_h]}),
                                ('CAPEX_w', {}),
                                ('OPEX_w', {}),
                                ('CAPEX_s', {}),
                                ('OPEX_s', {}),
